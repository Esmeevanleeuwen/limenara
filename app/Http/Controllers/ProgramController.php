...
class ProgramController
{
    private function validated(Request $request): array
    {
        return $request->validate([
            'title' => ['required', 'string', 'max:160'], 'summary' => ['required', 'string', 'max:2000'],
            'goals' => ['nullable', 'string', 'max:2000'], 'estimated_minutes' => ['required', 'integer', 'min:1', 'max:10000'],
            'lessons' => ['required', 'array', 'min:1', 'max:30'],
            'lessons.*' => ['required', 'array:title,type,body'],
            'lessons.*.title' => ['required', 'string', 'max:160'],
            'lessons.*.type' => ['required', Rule::in(['text', 'reflection'])],
    ...
    public function create()
    {
        ...
        return Inertia::render('staff/program-editor', ['program' => null]);
    }
    public function store(Request $request)
    {
        Gate::authorize('create', Program::class);
        $program = new Program($this->validated($request));
        $program->author_id = $request->user()->id;
        $program->status = 'draft';
        $program->save();
        return redirect('/werk/programmas/'.$program->id)->with('success', 'Concept aangemaakt.');
    }
    public function edit(Program $program)
    {
        Gate::authorize('update', $program);
        return Inertia::render('staff/program-editor', ['program' => $program->only(['id', 'title', 'summary', 'goals', 'estimated_minutes', 'lessons', 'status', 'revision'])]);
    }
    ...
    public function submit(Request $request, Program $program)
    {
        ...
        DB::transaction(function () use ($request, $program, $revision) {
            ...
            $locked->status = 'review'; $locked->revision++; $locked->save();
            AuditEvent::record($request->user()->id, 'program.submitted', $locked->id);
        });
        return back()->with('success', 'Concept ter beoordeling aangeboden.');
    }
    public function reviews(Request $request)
    {
        Gate::authorize('programs.review');
        return Inertia::render('admin/reviews', ['programs' => Program::where('status', 'review')->where('author_id', '!=', $request->user()->id)->latest()->get(['id', 'title', 'summary', 'goals', 'lessons', 'estimated_minutes', 'revision'])]);
    }
    ...
    public function returnDraft(Request $request, Program $program)
    {
        ...
        DB::transaction(function () use ($request, $program, $revision) {
            $locked = Program::whereKey($program->id)->lockForUpdate()->firstOrFail();
            abort_unless($locked->status === 'review' && $locked->revision === $revision, 409);
            $locked->status = 'draft'; $locked->revision++; $locked->save();
            AuditEvent::record($request->user()->id, 'program.returned', $locked->id);
        });
        return back()->with('success', 'Programma teruggezet naar concept.');
    }
}
